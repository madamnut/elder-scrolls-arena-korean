...
NPC 말투는 성별 하나로 결정하지 않는다. 역할·신분·상황·원문 태도를 함께 보고 변형 문장마다 별도로 번역한다. `TEMPLATE.DAT` 키 `0100~0114`는 역할별 첫 대면, `0115~0129`는 같은 역할의 재대면이며, 각 레코드의 `&` 구분 문장은 호감도 단계가 아니라 무작위 변형이다.

- 원문이 친절하면 친절하게, 적대적이면 적대적으로 옮긴다.
- 원문에 실제 모욕·욕설·성적 농담이 있으면 임의로 순화하지 않는다.
- 반대로 평범한 원문에 한국어 욕설을 새로 보태지 않는다.
- 여성 접객 NPC, 오만한 마법사, 겁먹거나 분개한 거지, 냉소적인 노예상처럼 역할별 성격을 유지한다.
- `0159~0180`의 소문·일거리·축일 답변과 `0259~0261`의 장소 안내는 여러 역할이 공유하므로 특정 성별이나 직업을 암시하지 않는 중립 구어체를 쓴다.

전체 직접 발화 범위와 진행 상태는 `translations/npc-dialogue-scope.json` 및 자동 생성 문서 `docs/NPC_DIALOGUE_SCOPE.ko.md`에서 관리한다. 역할·성별·문체와 변형별 태도표는 `translations/npc-dialogue-style.json`, 시민 신원 최종 말투는 `translations/npc-dialogue-revoice-overrides.json`에 있다. 거래 대사는 `npc-trade-revoice-by-index.json`, TEMPLATE의 추가 대사는 `npc-template-revoice-by-index.json`에서 원본 인덱스와 일대일로 관리한다.

장비점·매각·주점은 15개 화법이 제안·수락·재협상·결렬·절충의 다섯 단계에서 반복되고, 마법사 길드는 같은 구조를 9개 화법으로 사용한다. 한 화법은 거래 단계가 바뀌어도 같은 어휘 수준과 존대·반말을 유지한다.
